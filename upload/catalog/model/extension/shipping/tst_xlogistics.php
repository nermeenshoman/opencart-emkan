	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['tst_xlogistics'] = array(
				...
				'tax_class_id' => $this->config->get('tst_xlogistics_tax_class_id'),
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('tst_xlogistics_cost'), $this->config->get('tst_xlogistics_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
